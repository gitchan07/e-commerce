...
def add_transaction_detail(transaction_id):
    ...
    try:
        ...
        s.add(new_details)
        s.commit()
        detail_dict = new_details.__dict__.copy()
        detail_dict.pop("_sa_instance_state", None)
        return jsonify({"message": "Detail added", "detail": detail_dict}), 200
...
def get_transactions_details(transaction_id):
    ...
    try:
        ...
        details_dicts = [d.__dict__.copy() for d in details]
        for d_dict in details_dicts:
            d_dict.pop("_sa_instance_state", None)
        return jsonify(details_dicts), 200
    except Exception as e:
...
# PUT /transactions/{transaction_id}/details/<int:detail_id>
...
def update_transaction_detail(transaction_id, detail_id):
    ...
    try:
        ...
        if detail:
            ...
            detail_dict.pop("_sa_instance_state", None)
            return jsonify({"message": "Detail updated", "detail": detail_dict}), 200
        else:
